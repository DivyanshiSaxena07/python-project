...
const startGame = () => {
   ...
   const id=[{entry:'FirstName',type:'text'},{entry:'LastName',type:'text'},{entry:'Email',type:'email'},{entry:'PhoneNumber',type:'number'},{entry:'Designation',type:'text'},{entry:'Organization',type:'text'}]
        ...
        if(state.totalTime=='31'){
            confirm('Game Over')
               for(let x=0;x<id.length;x++){
                document.getElementById(field[x]).innerHTML = `${id[x].entry}<input required class='form-control' type="${id[x].type}" id=${id[x].entry} name="${id[x].entry}" placeholder="${id[x].entry}"  ></input>`; 
        ...
        if (flippedCards[0].innerText === flippedCards[1].innerText) {
            ...
            for(let x=0;x<id.length;x++){
                if(flippedCards[0].innerText==emojis[x]){
                    state.count++;
                    document.getElementById(field[x]).innerHTML = `${id[x].entry}<input required class='form-control' id=${id[x].entry} type="${id[x].type}" name="${id[x].entry}" placeholder="${id[x].entry}"  ></input>`; 
